...
- **✅ RESOLVED (verified 2026-08-25) — `mike-thompson` slug collision.** Zero records now share that bare slug. Separately, 26 bare-slug collisions still exist across the dataset (e.g. `mark-johnson` ×3 in OH/MN/AR), but they are **no longer a hazard**: `refine-records.py` indexes every candidate sharing a slug and resolves dossier keys via `slug@STATE`, SKIPPING an ambiguous bare slug with a loud warning instead of silently applying to the last match. Verified live: a dossier keyed on bare `mark-johnson` refines 0 records and reports the ambiguity.
- **✅ RESOLVED 2026-06-05 — `andre-dickens` slug collision (FL ghost).** Helper `dedup-andre-dickens.py` deleted the FL "ghost" duplicate record (state=FL with jurisdiction='Atlanta' — wrong; Atlanta is in GA) per the Stacy Garrity dedup precedent. The canonical GA record (state=GA, jurisdiction='City of Atlanta', confidence='evidence_local' from a prior batch) is preserved unchanged. Orphan `candidates/fl/andre-dickens.html` pruned. No merge needed since the FL ghost was archetype_curated and the GA canonical is already evidence-finalized.
