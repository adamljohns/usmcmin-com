...
- **⚠️ SLUG COLLISION — `rick-west` (BLOCKER, deferred 9 consecutive batches since 2026-06-02 1441, most recently 2026-06-04 2050):** Two active records share the bare slug `rick-west` — (1) Richard W. "Rick" West, Mayor of Chesapeake, VA (local tier, R) and (2) Rick West, OK State Representative (state tier, R). The `refine-records.py` engine keys by bare slug only (`by_slug = {c.get('slug'): c for c in sc['candidates']}`), so a dossier entry under `rick-west` would clobber the wrong record / mis-merge them. The Chesapeake mayor is the highest-priority VA local left in the queue (top of the priority sort each batch) and has been skipped 9 times (1441 + 1455 + 1530 + 1625 on 2026-06-02 + 1550 + 1710 + 1730 + 1750 + 2050 on 2026-06-04) pending a slug rename. Research is COMPLETE and stashed; would score ~67/100. **Fix:** write a small dedup-style helper that renames the VA mayor's slug (e.g. `rick-west` → `rick-west-chesapeake`), then run the refinement. The URL layout (`candidates/<state>/<slug>.html`) is unaffected because the two records live in different state subdirectories — only the engine's in-memory keying is the conflict.
