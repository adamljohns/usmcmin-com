(function (root, factory) {
  ...
  if (root) root.TmcFatherProgress = api;
})(typeof globalThis !== 'undefined' ? globalThis : this, function () {
  'use strict';

  const STORAGE_KEY = 'usmcmin:tmc-father:v1:progress';
  const TIME_KEY = 'usmcmin:tmc-father:v1:time';
  ...
  const TIMING_KEY = 'usmcmin:tmc-father:v1:timing';
  const CLOCK_KEY = 'usmcmin:tmc-father:v1:clock-visible';
  ...
  function sectionStorageKey(moduleId) {
    return `usmcmin:tmc-father:v1:sections:${moduleId}`;
  }

  function drillStorageKey(moduleId) {
    return `usmcmin:tmc-father:v1:drills:${moduleId}`;
  }
