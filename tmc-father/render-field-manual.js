 ...
 *
 * The version is derived from tmc-father/media-manifest.json, which is
...
const MEDIA_VERSION = (() => {
  ...
  try {
    ...
    throw new Error(
      'MEDIA_VERSION: cannot read tmc-father/media-manifest.json (' + err.message + '). ' +
      'Run: node tmc-father/build-media-manifest.js'
    );
  }
})();
...
function renderFieldManual({ module, course, layout, progressPanel, esc, prev, next }) {
  ...
  // workbook. Built by tmc-father/print/*.py.
  ...
  const insertLink = `<aside class="module-print" aria-labelledby="print-${module.number}-title">
      ...
      <p class="button-row"><a class="button" href="/downloads/tmc-father/The_Husband_Course_Module_${pad}_INSERT.pdf">Download the module ${module.number} insert (PDF)</a></p>
      <p class="module-print-alt">Taking the whole course on paper? <a href="/downloads/tmc-father/The_Husband_Course_FIELD_WORKBOOK.pdf">The Husband&rsquo;s Field Workbook</a> carries all seven modules.</p>
    </aside>`;
