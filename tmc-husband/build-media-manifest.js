 ...
 *
 *   node tmc-husband/build-media-manifest.js
 *
...
const ROOT = path.join(__dirname, '..', 'assets', 'media', 'tmc-husband');
...
if (!count) {
  console.error(
    'No media found under assets/media/tmc-husband.\n' +
    'Refusing to write an empty manifest — that would bust every reader\'s cache.\n' +
    'Restore the media first (see tmc-husband/sync-media.sh).'
  );
  process.exit(1);
}
